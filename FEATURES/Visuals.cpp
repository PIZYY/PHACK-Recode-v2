	...
	for (int i = 1; i <= 65; i++)
	{
		...
		if (RENDER::WorldToScreen(pos3D, pos) && RENDER::WorldToScreen(top3D, top))
		{
			...
			//	if (SETTINGS::settings.weaponstype == 2) DrawWeapon2(entity, weapon_color, i, pos, top);
				...
				if (SETTINGS::settings.name_bool) DrawLBYBar(entity, lby);
...
void CVisuals::DrawWeapon(SDK::CBaseEntity* entity, CColor color, int index, Vector pos, Vector top)
{
	...
	else if (c_baseweapon->GetItemDefenitionIndex() == SDK::ItemDefinitionIndex::WEAPON_USPS)
		RENDER::DrawF(pos.x, is_teammate ? distanceOn2 : distanceOn, FONTS::visuals_esp_font, true, true, color, "USP-S");
	else if (c_baseweapon->GetItemDefenitionIndex() == SDK::ItemDefinitionIndex::WEAPON_M4A1S)
...
void CVisuals::DrawInfo(SDK::CBaseEntity* entity, CColor color, CColor alt, Vector pos, Vector top)
{
	...
	if (SETTINGS::settings.info_bool && !is_teammate)
	{
		...
		stored_info.push_back(std::pair<std::string, CColor>(GLOBAL::resolver_mode, alt));
	}
...
void CVisuals::DrawLBYBar(SDK::CBaseEntity* entity, CColor color)
{
	...
	//RENDER::Textf((20 - (screen_height - 15)) + 10, 10 + (screen_height - 20) + 4, width, 10 + (screen_height - 10) + 6, "Desync",CColor(255, 255, 255));
}
...
void CVisuals::watermark()
{
	...
	RENDER::Textf(screen_width - 187, 4, CColor(255, 255, 255, 255), FONTS::visuals_name_font, XorStr("PHACK | Fps: %d | in: %d ms | out: %d ms "), getfps(), getping(), getping2());
}
...
void CVisuals::DrawIndicator()
{
	...
	{
		if (breaking_lc)
			RENDER::DrawF(10, screen_height / 2 - 20, FONTS::menu_combobox_font, false, false, CColor(147, 189, 29), "LC"); // greem

		else
			RENDER::DrawF(10, screen_height / 2 - 20, FONTS::menu_combobox_font, false, false, CColor(252, 17, 17), "LC"); // red
	}
	else
	{
	...
	if (SETTINGS::settings.lag_bool)
	{
		...
		if (!net_channel)
			...
			char choks[64];
			sprintf_s(choks, "Choke: %d", net_channel->m_nChokedPackets);
			RENDER::DrawF(6, screen_height  / 2 - 50, FONTS::menu_combobox_font, false, false, CColor(255, 255, 255, 255), choks);
...
void CVisuals::GameSense()
{
	...
	if (GetTickCount() > lastTime)
	{
		...
		if (kek > 10) {
			...
			setClanTag(NoNamersAnim[autism], "phack.pw");
