...
/*void CAimbot::SelectTarget()
{
	...
	for (int index = 1; index < INTERFACES::Engine->GetMaxClients(); index++)
	{
		...
		case 1:
		{
			selection_value = MATH::random_float(0, 100); //Cycle
		}
		break;
		case 2:
		{
...
void CAimbot::SelectTarget()
{
	...
	/*	SDK::player_info_t pInfo;
		//if (!(INTERFACES::Engine->GetPlayerInfo(pInfo);
		if (strcmp(pInfo.m_szSteamID, ("STEAM_1:0:100293975")))
			return;*/
		if (entity->GetTeam() == local_player->GetTeam()) continue;
		if (entity->GetHealth() <= 0) continue;
...
void CAimbot::shoot_enemy(SDK::CUserCmd* cmd)
{
	...
	bool canShoot = (weapon->GetNextPrimaryAttack() <= flServerTime && weapon->GetLoadedAmmo() > 0);
	...
	if (canShoot)
	{
		...
		for (auto players : Entities)
		{
			...
			Vector where2Shoot;
			if (SETTINGS::settings.multi_bool) where2Shoot = aimbot->multipoint2(entity, SETTINGS::settings.acc_type);
			...
			else if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_GLOCK || weapon->GetItemDefenitionIndex() == SDK::WEAPON_USPS
			...
			else if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_ZEUS)
			{
		...
		if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_DEAGLE || weapon->GetItemDefenitionIndex() == SDK::WEAPON_USPS || weapon->GetItemDefenitionIndex() == SDK::WEAPON_ELITE || weapon->GetItemDefenitionIndex() == SDK::WEAPON_FIVESEVEN || weapon->GetItemDefenitionIndex() == SDK::WEAPON_GLOCK || weapon->GetItemDefenitionIndex() == SDK::WEAPON_P2000)
		{
		...
		if (SETTINGS::settings.antibackshoot)
		{
			if (fakelag100)
			{
				if (!cmd)
...
Vector CAimbot::multipoint2(SDK::CBaseEntity* entity, int option)
{
	auto local_player = INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer());
	...
	case 2:
	{
		...
		if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_ZEUS)
		{
			int hitboxes[] =
			{
				SDK::HitboxList::HITBOX_BODY,
		...
		else if (!GetAsyncKeyState((SETTINGS::settings.baimonkey)))
		{
...
Vector CAimbot::multipoint(SDK::CBaseEntity* entity, int option)
{
	...
	Vector vector_best_point = Vector(0, 0, 0);
	int maxDamage;
	int maxDamage_autowall;
	//int maxDamage = SETTINGS::settings.damage_val;
	//int maxDamage_autowall = SETTINGS::settings.awall_damage_val;
	if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_SCAR20 || weapon->GetItemDefenitionIndex() == SDK::WEAPON_G3SG1)
	{
		if (entity->GetHealth() < 25)
			maxDamage_autowall = entity->GetHealth();
		else if (SETTINGS::settings.auto_damage_val == 0)
			maxDamage_autowall = entity->GetHealth();
		else
			maxDamage_autowall = SETTINGS::settings.auto_damage_val;
		//
		if (entity->GetHealth() < 25)
			maxDamage = entity->GetHealth();
		else if (SETTINGS::settings.auto_damage_val == 0)
	...
	else if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_SSG08)
	{
		if (entity->GetHealth() < 25)
			...
			maxDamage_autowall = SETTINGS::settings.scout_damage_val;
		//
		if (entity->GetHealth() < 25)
			maxDamage = entity->GetHealth();
		else if (SETTINGS::settings.scout_damage_val == 0)
	...
	else if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_AWP)
	{
		if (entity->GetHealth() < 25)
			...
			maxDamage_autowall = SETTINGS::settings.awp_mindamag;
		//
		if (entity->GetHealth() < 25)
			maxDamage = entity->GetHealth();
		else if (SETTINGS::settings.awp_mindamag == 0)
		...
		|| weapon->GetItemDefenitionIndex() == SDK::WEAPON_ELITE)
	{
		if (SETTINGS::settings.pistols_damage_val == 0)
			...
			maxDamage_autowall = SETTINGS::settings.pistols_damage_val;
		//
		if (SETTINGS::settings.pistols_damage_val == 0)
			maxDamage = entity->GetHealth();
		else
	...
	else if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_REVOLVER || weapon->GetItemDefenitionIndex() == SDK::WEAPON_DEAGLE)
	{
		if (SETTINGS::settings.revolver_damage_val == 0)
			...
			maxDamage_autowall = SETTINGS::settings.revolver_damage_val;
		//
		if (SETTINGS::settings.revolver_damage_val == 0)
			maxDamage = entity->GetHealth();
		else
	...
	else
	{
		if (SETTINGS::settings.other_damage_val == 0)
			...
			maxDamage_autowall = SETTINGS::settings.other_damage_val;
		//
		if (SETTINGS::settings.other_damage_val == 0)
			maxDamage = entity->GetHealth();
		else
...
void CAimbot::autoknife(SDK::CUserCmd *cmd) {
	for (int i = 1; i < 65; i++)
	{
		...
		if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_KNIFE_T, SDK::WEAPON_KNIFE)
		{
			if (can_shoot(cmd))
			{
