...
public:
	...
	static void fake_stand(SDK::CUserCmd* cmd)
	{
		if (GetAsyncKeyState(SETTINGS::settings.fakestand))
		{
			if (!cmd)
			{
				return;
	...
	{
		switch (SETTINGS::settings.fakeduckver)
		{
		case 0: {
		...
		case 1: {
			if (GetAsyncKeyState(SETTINGS::settings.Fakecrouchkey))
			{
				...
				if (NetChannel->m_nChokedPackets <= 6)
					cmd->buttons &= ~IN_DUCK;
				else
					cmd->buttons |= IN_DUCK;

				if (NetChannel->m_nChokedPackets < 14)
					 GLOBAL::should_send_packet = false; // choke 
				else
					GLOBAL::should_send_packet = true; // send packet 
			}
			/*if (GetAsyncKeyState(SETTINGS::settings.Fakecrouchkey))
			{
				auto NetChannel = INTERFACES::Engine->GetNetChannel();
				int limit = SETTINGS::settings.Fakelagamount / 2;
